...
def _main():
    ...
    log_dir = 'logs/20181017/'
    classes_path = 'Data/object_classes.txt'

    anchors_path = 'model_data/yolo_anchors.txt'
    class_names = get_classes(classes_path)
    anchors = get_anchors(anchors_path)
    input_shape = (288, 288)  # multiple of 32, hw
    model = create_model(input_shape, anchors, len(class_names),weights_path='model_data/yolo_weights.h5')
    train(model, annotation_path_train,annotation_path_val, input_shape, anchors, len(class_names), log_dir=log_dir)




def train(model, annotation_path_train,annotation_path_val, input_shape, anchors, num_classes, log_dir='logs/'):
    ...
    model.fit_generator(data_generator_wrap(lines_train, batch_size, input_shape, anchors, num_classes),
    ...
    model.save_weights(log_dir + 'trained_weights_first.h5')
    ...
    model.fit_generator(data_generator_wrap(lines_train, batch_size, input_shape, anchors, num_classes),
                        steps_per_epoch=max(1, num_train // batch_size),
                        validation_data=data_generator_wrap(lines_val, batch_size, input_shape, anchors,num_classes),
                        validation_steps=max(1, num_val // batch_size),
                        epochs=51,
                        initial_epoch=50,
                        callbacks=[logging, checkpoint,reduce_lr,early_stopping])
    model.save_weights(log_dir + 'trained_weights_finally.h5')
