...
def _main():
    ...
    log_dir = 'logs/20181029/'
    classes_path = 'model_data/object_classes2018_10_29.txt'
    ...
    model = create_model(input_shape, anchors, num_classes,load_pretrained=False,weights_path='logs/20181026/yolov3_20181026_0.h5')
    train_num = 0
    model_name='yolov3_20181029_'
    save_model_name = model_name+ str(train_num) + '.h5'
    train(model, annotation_path_train,annotation_path_val, input_shape, anchors, len(class_names),save_model_name,
          log_dir=log_dir,random=random,epoch_num=epoch_num)

    #第二阶段多尺度训练
    #第一项为训练尺寸，第二项为是否数据增强，第三项为训练的初始epoch和总的epoch
    muti_scales = [[(416, 416), True,(40,70)], [(608, 608), True,(45,75)],[(320,320), False,(50,80)],
                   [(416, 416), True,(55,85)],[(320,320), True,(60,90)],[(608,608), False,(65,125)]]
    # muti_scales=[(416,416),(608,608),(320,320),(416,416),(608,608),(320,320),(608,608)]
    # muti_scales = [(256, 256), (288, 288), (416, 416), (608, 608), (288, 288)]



    for muti_scale in muti_scales:
        input_shape,random,epoch_num=muti_scale
        ...
        save_model_name = model_name+ str(train_num) + '.h5'
        train_mutiscale(model, annotation_path_train, annotation_path_val, input_shape, anchors, len(class_names),
                        save_model_name,log_dir=log_dir,random=random,epoch_num=epoch_num)
...
def train(model, annotation_path_train,annotation_path_val, input_shape, anchors, num_classes,save_model_name,
          log_dir='logs/',random=False,epoch_num=(0,35)):
    ...
    p_model.fit_generator(data_generator_wrap(lines_train, batch_size, input_shape, anchors, num_classes,random=random),
                        steps_per_epoch=max(1, num_train // batch_size),
                        validation_data=data_generator_wrap(lines_val, batch_size, input_shape, anchors,
                                                            num_classes,random=random),
                        validation_steps=max(1, num_val // batch_size),
                        epochs=epoch_num[1],
                        initial_epoch=epoch_num[0],
                        callbacks=[logging, checkpoint])
    model.save_weights(log_dir + save_model_name)


def train_mutiscale(model, annotation_path_train, annotation_path_val, input_shape, anchors, num_classes,save_model_name,
                    log_dir='logs/',random=True,epoch_num=(50,80)):
    p_model = multi_gpu_model(model, gpus=2)
